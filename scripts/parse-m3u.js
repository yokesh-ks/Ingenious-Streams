...
const M3U_URL = 'https://raw.githubusercontent.com/yokesh-ks/iptv/master/streams/in.m3u';
const OUTPUT_FILE = path.join(__dirname, '../data/channels.json');

// Category mapping patterns
const CATEGORY_PATTERNS = {
  news: [
    'aaj tak', 'abp', 'india news', 'republic', 'times now', 'ndtv', 'news18',
    'zee news', 'news24', 'tv9', 'dd news', 'news', 'cnbc', 'et now'
  ],
  entertainment: [
    'colors', 'star plus', 'sony', 'zee tv', '&tv', 'sab', 'star bharat',
    'dangal', 'rishtey', 'shemaroo', 'entertainment'
  ],
  movies: [
    'cinema', 'movies', 'movie', 'gold', 'max', 'pix', 'flix', 'bollywood',
    'pictures'
  ],
  music: [
    'music', '9xm', '9x jalwa', 'b4u', 'mtv', 'vh1', 'mastii', 'sangeet',
    'hungama', 'zoom'
  ],
  sports: [
    'sport', 'cricket', 'football', 'tennis', 'espn', 'star sports', 'sony ten',
    'eurosport'
  ],
  kids: [
    'kids', 'nick', 'pogo', 'disney', 'cartoon', 'discovery kids', 'hungama tv',
    'sony yay'
  ],
  devotional: [
    'aastha', 'sanskar', 'bhakti', 'devotional', 'god', 'spiritual', 'iskcon',
    'shubh'
  ],
  regional: [
    'tamil', 'telugu', 'kannada', 'malayalam', 'marathi', 'bangla', 'bengali',
...
const FEATURED_CHANNELS = [
  'Aaj Tak', 'ABP News', 'NDTV India', 'Zee News', 'India TV',
  'Colors HD', 'Star Plus HD', 'Sony TV HD', 'Zee TV HD',
  'Zee Cinema HD', 'Star Gold HD', 'Sony Max HD', '&pictures HD',
  '9XM', 'MTV Beats HD',
  'Star Sports 1 HD', 'Sony Ten 1 HD',
  'Discovery Channel', 'National Geographic',
  'Sony Sab HD', 'Star Bharat HD'
];
...
function extractLanguage(info) {
  const languages = ['Hindi', 'English', 'Tamil', 'Telugu', 'Kannada', 'Malayalam', 'Bengali', 'Marathi', 'Punjabi', 'Gujarati'];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('#EXTINF:')) {
      ...
      if (nameMatch && i + 1 < lines.length) {
        ...
        // Check if featured
        const isFeatured = FEATURED_CHANNELS.some(fc =>
...
async function main() {
  try {
    ...
    console.log(`Found ${channels.length} channels in ${categories.length} categories`);
    ...
    console.log(`- Featured channels: ${featuredChannelIds.length}`);
    console.log(`- Categories: ${categoryDefinitions.map(c => c.name).join(', ')}`);
